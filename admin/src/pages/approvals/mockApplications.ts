/**
 * Stand-in for the volunteer applications endpoint — ported from
 * mobile/src/services/volunteer-application.mock.ts so both admin surfaces
 * show the same data.
 *
 * TODO: replace with a real API module calling
 * `GET /api/admin/volunteer-applications`. Approving and rejecting currently
 * only mutate local page state — there is no VolunteerApplication model or
 * admin endpoint behind the approvals pages yet.
 */
...
export interface VolunteerApplication {
  ...
  profileImage?: string;
  availability: string[];
  idDocument: { fileName: string; uri: string };
  /** ISO timestamp. */
  appliedAt: string;
...
// TODO: Mock data. Needs the same VolunteerApplication backend as the approvals page.
export const MOCK_APPROVAL_HISTORY: ApprovalHistoryEntry[] = [
  { id: 'h1', name: 'Robert Davis', role: 'Volunteer Driver', time: '2 days ago', status: 'Approved' },
  { id: 'h2', name: 'Emily Chen', role: 'Community Outreach', time: '4 days ago', status: 'Rejected' },
];
