...
export default function EditRequestScreen() {
  ...
  const { getAppointmentById, updateRequest, submitting } = useAppointments();
  ...
  useEffect(() => {
    async function loadAppointmentDetails() {
      ...
      try {
        ...
        if (item) {
          setTitle(item.title || '');
          setTaskType((item.taskType as TaskType) || 'Grocery Shopping');
          setUrgency((item.urgency as UrgencyLevel) || 'Normal');
